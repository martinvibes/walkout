...
reads video once and carefully. The request quota is counted per model, so
...
conflict only exists if you insist on running both under one interpreter, so
the server lives in its own `.venv-mcp` (`make mcp-server`), which the
...
footage, which is where Gemini comes in. Pretending otherwise would be the
whole product lying.
